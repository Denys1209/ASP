...
public class NovelsController : Controller
{
    ...
    public async Task<IActionResult> Create(
        [Bind("Title,Description,ReleaseDate,HowManyPages,SelectedCategoryIds,SelectedNovelistIds")] NovelViewModel viewModel, 
        CancellationToken cancellationToken)
    {
        var dto = new FilterPaginationDto();
        var categories = await _categoriesService.GetAllModelsByIdsAsync(viewModel.SelectedCategoryIds, cancellationToken);
        var novelists = await _novelistsService.GetAllModelsByIdsAsync(viewModel.SelectedNovelistIds, cancellationToken);
        Novel novel = new()
        {
            Categories = categories?.ToList() ?? new List<Category>(),
            Novelists = novelists?.ToList() ?? new List<Novelist>(),
